...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $date_lost_value   = htmlspecialchars($date_lost, ENT_QUOTES, 'UTF-8');
    $location_value    = htmlspecialchars($location, ENT_QUOTES, 'UTF-8');

    // Basic validation
    if (empty($item_name) || empty($description) || empty($category) || empty($date_lost)) {
        $message = "Please fill in all required fields.";
    } elseif (strlen($description) > 500) {
        $message = "Description cannot exceed 500 characters.";
    } elseif (!preg_match("/^\d{4}-\d{2}-\d{2}$/", $date_lost)) {
        ...
        if (empty($message)) {
            try {
                $stmt = $conn->prepare("
                    ...
                    (item_name, description, image_path, category, date_lost, location, user_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $item_name,
                ...
                $item_name_value = $description_value = $category_value = $date_lost_value = $location_value = '';
...
<head>
    ...
    <title>Report Lost Item | Lost & Found KyU</title>
...
<body>
    ...
    <div class="container report-wrap">
        <div class="card report-card">
            <div class="report-header">
                ...
                <h3 class="h5 mb-1 fw-bold">Report a Lost Item</h3>
                <p class="mb-0 small opacity-75">Share clear details so the community can help recover your belonging quickly.</p>
            </div>

            <div class="card-body p-4 p-lg-5">
                <div class="stepper mb-3" id="lostStepper">
                    <div class="step active" id="lost-step-1">
                        <div class="bullet">1</div>
                        <div><strong>Item details</strong><br><small class="text-muted">Name, type, date, place</small></div>
                    </div>
                    <div class="step" id="lost-step-2">
                        <div class="bullet">2</div>
                        <div><strong>Photo</strong><br><small class="text-muted">Attach a clear picture</small></div>
                    </div>
                    <div class="step" id="lost-step-3">
                ...
                <div class="progress mb-4" role="progressbar" aria-label="Form progress" aria-valuemin="0" aria-valuemax="100">
                    <div class="progress-bar" id="lostProgressBar" style="width: 30%"></div>
                </div>
                ...
                <form method="POST" enctype="multipart/form-data" id="lostForm" novalidate>
                    <div class="form-section">
                        <h5 class="mb-3"><i class="fas fa-clipboard-list me-2 text-primary"></i>Item Details</h5>

                        <div class="mb-3">
                            <label for="item_name" class="form-label">Item Name <span class="text-danger">*</span>
                                <i class="fas fa-circle-info text-muted ms-1" data-bs-toggle="tooltip" title="Use a clear name someone can recognize quickly."></i>
                            </label>
                        ...
                        <div class="mb-3">
                            <label for="description" class="form-label">Description <span class="text-danger">*</span>
                                <i class="fas fa-circle-info text-muted ms-1" data-bs-toggle="tooltip" title="Mention color, brand, unique marks, and useful identifying details."></i>
                            </label>
                            <div class="input-shell">
                                ...
                                <textarea id="description" name="description" class="form-control" required maxlength="500"
                                          placeholder="Describe your item in detail (color, model, markings, contents, stickers, damage, etc.)"><?php echo $description_value; ?></textarea>
                            </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">Category <span class="text-danger">*</span>
                                <i class="fas fa-circle-info text-muted ms-1" data-bs-toggle="tooltip" title="Choose the closest category to improve search matching."></i>
                            </label>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-shell">
                                    ...
                                    <input type="date" id="date_lost" name="date_lost" class="form-control" required
                                           ...
                                           value="<?php echo $date_lost_value; ?>">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <label for="location" class="form-label">Location Last Seen
                                    <i class="fas fa-circle-info text-muted ms-1" data-bs-toggle="tooltip" title="Examples: Main Library, Bus Park, Lecture Room B3."></i>
                                </label>
                                <div class="input-shell">
                                    <i class="fas fa-location-dot field-icon"></i>
                                    <input type="text" id="location" name="location" class="form-control" maxlength="140"
                                           placeholder="e.g., Main Library, Bus Park"
                    ...
                    <div class="d-flex flex-column flex-md-row gap-2 mt-4">
                        <button type="submit" class="btn btn-theme flex-grow-1" id="submitLostBtn">
    ...
    <script>
        ...
        const progressBar = document.getElementById('lostProgressBar');

        const step1 = document.getElementById('lost-step-1');
        const step2 = document.getElementById('lost-step-2');
        const step3 = document.getElementById('lost-step-3');
        ...
        function updateSteps() {
            ...
            const dateFilled = document.getElementById('date_lost').value.trim() !== '';
        ...
        ['item_name', 'description', 'date_lost', 'location'].forEach((id) => {
